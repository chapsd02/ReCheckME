...
const SkeletonLoader: React.FC = () => {
  ...
  const ShimmeringDiv: React.FC<{className?: string}> = ({ className }) => (
    <div className={`bg-gray-300 bg-gradient-to-r from-gray-300 via-gray-200 to-gray-300 bg-[length:1000px_100%] animate-shimmer ${className || ''}`}></div>
  );

  return (
    <div className="bg-white border border-gray-200 shadow-lg rounded-2xl overflow-hidden w-full">
        <div className="p-5 bg-gray-200">
            <ShimmeringDiv className="h-7 w-3/5 rounded-md" />
        </div>

        <div className="bg-gray-100 m-4 rounded-xl p-5 text-center space-y-3">
